...
@nb.njit(["b1[:](i8[:, :], b1[:])",
          "b1[:](i4[:, :], b1[:])"], cache=True, parallel=True)
...
def FilterDetection(detection, min_separation, min_duration):
    ...
    for ind, _ in np.ndenumerate(intervals):
        intervals[ind] = filter_intervals(_giveIntervals(detection[ind]), min_separation, min_duration)
        filtered_detection[ind] = project_intervals(intervals[ind], filtered_detection[ind])
    return filtered_detection, intervals
