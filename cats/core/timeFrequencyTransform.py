...
class STFT_Operator: 
    def __init__(self, window, overlap=0.5, dt=1, backend='scipy', nfft=None, padtype='reflect', **kwargs):
        ...
        """
        self.window     =   window
        self.nperseg    =   nperseg = len(window)
        ...
        self.nfft       =   2**int(np.ceil(np.log2(nperseg))) if self.nfft < 0 else max(self.nfft, nperseg)
    ...
    def Inverse(self, C):
        """
            ...
            C : numpy array : shape (..., Nf, Nt), where `Nf` frequencies, `Nt` time frames. 
                              `C` can have any dimesional structure for any number of spectrograms, 
