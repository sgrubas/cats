...
class CATSDetector(CATSBaseSTFT):
    def __init__(self, dt_sec, stft_window_sec, stft_overlap, stft_nfft, minSNR, stationary_frame_sec,
                 cluster_size_t_sec, cluster_size_f_Hz, cluster_distance_t_sec,
                 cluster_distance_f_Hz, min_t_separation_sec, clustering_with_SNR=True, clustering_multitrace=False,
                 cluster_size_trace=None, cluster_distance_trace=None,
    ...
    def detect(self, x):
        ...
        C = (K > 0)
        c = C.max(axis=-2)
        detection = RemoveGaps(c, self.min_t_separation_len, self.cluster_size_t_len)
