...
class SNRDetector:
    def __init__(self, dt_sec, stft_window_sec, stft_nfft, minSNR, stationary_frame_sec,
                 max_dt_gap_sec, min_dt_width_sec, stft_overlap,
    ...
    def detect(self, x):
        ...
        Eta = BEDATE(PSD, frames=stationary_intervals, minSNR=self.minSNR, Q=self.date_Q)
        Sgm = EtaToSigma(Eta, self.minSNR)
        B = ThresholdingSNR(PSD, Sgm, stationary_intervals, self.minSNR)
        c = B.max(axis=-2)
        detection = ProjectFilterIntervals(c, stft_time, self.max_dt_gap_sec, self.min_dt_width_sec, stft_time)

        kwargs = {"signal": x, "spectrogram": X, "noise_thresholding": Eta, "noise_std": Sgm,
                  "binary_spectrogram": B, "binary_projection": c, "final_detection": detection,
                  "time": time, "stft_time": stft_time, "stft_frequency": self.stft_frequency,
                  "stationary_intervals": stationary_intervals}
...
class SNRDetectionResult:
    ...
    def plot(self, ind, figsize=250, cmap='viridis'):
        ...
        PSD = np.abs(self.spectrogram[ind])
        SNR = self.binary_spectrogram[ind] * PSD

        fig0 = hv.Curve((self.time, self.signal[ind]), kdims=[t_dim], vdims=A_dim,
                        label='0. Input data: $x_n$').opts(xlabel='', linewidth=0.2)
        fig1 = hv.Image((self.stft_time, self.stft_frequency, PSD), kdims=[t_dim, f_dim],
                        label='1. Spectrogram: $|X_{k,m}|$')
        fig2 = hv.Image((self.stft_time, self.stft_frequency, SNR), kdims=[t_dim, f_dim],
                        label='2. SNR spectrogram trimmed by B-E-DATE: $B_{k,m} \cdot |X_{k,m}| / \sigma_{j,m}$')
        fig3 = hv.Curve((self.stft_time, self.final_detection[ind] * 1), kdims=[t_dim], vdims='Classification',
                        label='4. Projection: $c_k$').opts(xlabel='Time (s)')
        ...
        fig = (fig0 + fig1 + fig2 + fig3).opts(fig_size=figsize, shared_axes=True,
                                               vspace=0.4, aspect_weight=0,
                                               sublabel_format='').opts(hv.opts.Image(**spectr_kwargs),
                                                                        hv.opts.Curve(**curve_kwargs))
        return fig.cols(1)
